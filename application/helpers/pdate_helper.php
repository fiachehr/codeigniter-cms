...
function greToJal($date){
	
	$chDate = strtotime($date);
	return pdate("Y-m-d",$chDate);

}

function jalToGre($date){
...
function compareDate($date1,$date2){		
	...
	$d2 = strtotime($date2);
	$time = round(($d2-$d1) / 60);
	$day = floor($time / (24 * 60));
	$hours = $time - ($day * 24 * 60); 
	$hour = floor($hours / 60);
	$minutes = $time - (($hour * 60) + ($day * 24 * 60));
	return array("day" => $day , "hour" => $hour , "min" => $minutes);
	
}

function getNameDate($value,$type){
	if($type == "D"){
		$pdateWeekName=array("Sat"=>"شنبه","Sun"=>"یکشنبه","Mon"=>"دوشنبه","Tud"=>"سه شنبه","Wed"=>"چهارشنبه","Thu"=>"پنج شنبه","Fri"=>"جمعه");
		return $pdateWeekName[$value];
	}else{
		$pdateMonthName=array("فروردین","اردیبهشت","خرداد","تیر","مرداد","شهریور","مهر","آبان","آذر","دی","بهمن","اسفند");
		return $pdateMonthName[$value-1];
	}
}
